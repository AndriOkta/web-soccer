...
export class ScoringsComponent implements OnInit {
  ...
  constructor( private _soccerSevice: SoccerService ) { 
  ...
  private getSchedule(){
    if(this.UsingAsync) {
      let xx = this._soccerSevice.getScheduleAsync();
        xx.then((Schedules:Schedule[])=> this.MySchedule = Schedules);
    }else{
      this.MySchedule = this._soccerSevice.getSchedule();
    }
  }
